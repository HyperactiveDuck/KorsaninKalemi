    ...
    <div className="min-h-screen bg-obsidian text-softpearl font-sans selection:bg-crimson selection:text-white flex flex-col">
      ...
      <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-obsidian/90 backdrop-blur-md py-4 border-b border-gray-800' : 'bg-transparent py-8'}`}>
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          <div className="hidden md:flex gap-12 text-xs font-bold uppercase tracking-widest items-center text-white">
            <a href="#philosophy" onClick={(e) => handleNavClick(e, 'philosophy')} className={`hover:text-crimson transition-colors shadow-sm ${currentPage === 'LIBRARY' ? 'opacity-70' : ''}`}>Ben Kimim</a>
            <a href="#approach" onClick={(e) => handleNavClick(e, 'approach')} className={`hover:text-crimson transition-colors shadow-sm ${currentPage === 'LIBRARY' ? 'opacity-70' : ''}`}>Kitaplarım</a>
            <a href="#services" onClick={(e) => handleNavClick(e, 'services')} className={`hover:text-crimson transition-colors shadow-sm ${currentPage === 'LIBRARY' ? 'opacity-70' : ''}`}>Etkinlikler</a>
      ...
      {isMenuOpen && (
        <div className="fixed inset-0 bg-obsidian z-40 flex flex-col items-center justify-center space-y-8 text-xl font-serif italic animate-in fade-in slide-in-from-top-10 duration-300">
          <a href="#philosophy" onClick={(e) => handleNavClick(e, 'philosophy')} className="hover:text-crimson">Ben Kimim</a>
          <a href="#approach" onClick={(e) => handleNavClick(e, 'approach')} className="hover:text-crimson">Kitaplarım</a>
          <a href="#services" onClick={(e) => handleNavClick(e, 'services')} className="hover:text-crimson">Etkinlikler</a>
      ...
      <main className="flex-grow">
        {currentPage === 'HOME' ? (
          <>
            ...
            {/* Centered Callout Container (Philosophy - Ben Kimim) */}
